...
# Run examples
./bin/canopennode_blank    # Basic CANopenNode example
./bin/quick_scan           # CANopen device scanner
./bin/pp_mode_control 2    # CiA402 PP mode controller (specify motor ID)
```

### Available Targets

- **canopennode** - Static library containing CANopenNode core functionality
- **canopennode_blank** - Basic CANopenNode example application
- **quick_scan** - CANopen device scanner utility
- **pp_mode_control** - CiA402 PP mode controller with manual motor ID input
...
The library will be installed to `/usr/local/lib/` and headers to `/usr/local/include/canopennode/`.

### Motor Node ID Configuration

The `pp_mode_control` program now supports **manual motor ID input**:

- **Command line**: `./bin/pp_mode_control 2` (specify node ID directly)
- **Interactive**: `./bin/pp_mode_control` (prompts for node ID input)
- **Device scanner**: Use `quick_scan` tool to discover CANopen devices

For detailed motor ID configuration, see [MOTOR_ID_GUIDE.md](MOTOR_ID_GUIDE.md).

For usage examples and troubleshooting, see [USAGE_EXAMPLES.md](USAGE_EXAMPLES.md).

For a comprehensive project overview and system architecture, see [PROJECT_DESCRIPTION.md](PROJECT_DESCRIPTION.md).
...
--------------
 ...
 - **example/** - Directory with basic examples, should compile on any system.
   ...
   - **quick_scan.c** - CANopen device scanner utility.
   - **pp_mode_control.c** - CiA402 PP mode controller example.
   ...
   - **ZeroErr Driver_V1.5.eds** - Example EDS file for motor control.
   - **CMakeLists.txt** - CMake configuration for examples.
   - **README_VSCODE.md** - VSCode configuration guide.
   - **README_pp_mode_control.md** - PP mode control documentation.
 ...
 - **CMakeLists.txt** - CMake configuration for building the library and examples.
